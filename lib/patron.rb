class Patron
  ...
  def link_patron_book(book_id)
    ...
    if book != nil
      previous = DB.exec("SELECT * FROM books_patrons WHERE book_id = #{book_id} AND patron_id = #{@id}").first
      if previous == nil
        ...
        DB.exec("UPDATE books_patrons SET checkout_date = 'now()'")
        DB.exec("UPDATE books_patrons SET return_date = CURRENT_DATE + INTERVAL '7 days'")
      end
    else
      nil
    end
